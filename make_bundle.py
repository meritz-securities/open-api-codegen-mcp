...
"""플랫폼별 Claude Desktop 설치 파일(.mcpb)을 만듭니다.
    ...
    python3 make_bundle.py darwin-arm64     # bin/meritz-codegen-mcp 가 있어야 한다
    python3 make_bundle.py darwin-x64
    python3 make_bundle.py win32-x64        # bin/meritz-codegen-mcp.exe
...
PLATFORMS = {
    "darwin-arm64": ("darwin", "meritz-codegen-mcp"),
    "darwin-x64":   ("darwin", "meritz-codegen-mcp"),
    "win32-x64":    ("win32",  "meritz-codegen-mcp.exe"),
}
COPY = ["README.md", "LICENSE", "DISCLAIMER.md", "SECURITY.md", "THIRD-PARTY-NOTICES.txt"]
...
def main() -> int:
    ...
    if not src_bin.exists():
        print(f"{src_bin} 가 없습니다. ./build_binary.sh 를 먼저 실행하세요.")
        return 1
