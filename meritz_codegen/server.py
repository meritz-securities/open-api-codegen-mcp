...
# corrections.json 을 도구 응답에 실을 때 내보낼 칸.
#
# 예전에는 dict 를 통째로 넘겼고, 그래서 감사 기록(언제 확인했나·무엇을 못
# 봤나)이 고객 응답에 그대로 실려 나갔다. 배포 데이터는 이미 정리했지만
# 코드가 통째로 넘기는 한 같은 일이 다시 생긴다. 여기 없는 칸은 나가지 않는다.
PUBLIC_CORRECTION_FIELDS = (
    "kind", "symptom", "portal_example", "actual",
    "if_you_follow_the_portal", "workaround", "caution",
    "working_example", "actual_parameter",
)


def public_correction(fix: dict) -> dict:
    """고객에게 내보낼 칸만 남긴다."""
    return {k: fix[k] for k in PUBLIC_CORRECTION_FIELDS if k in fix}
...
def get_api_detail(
    ...
    if fixes:
        out["response_handling_notes"] = [public_correction(f) for f in fixes]
...
def api_conventions() -> dict:
    return {
        ...
        "known_issues": [public_correction(c)
                         for c in CATALOG.corrections.get("corrections", [])],
...
# 프롬프트 문구는 여기 하나뿐이다.
#
# Claude Desktop 은 번들 확장의 프롬프트를 "선언된 고정 문구"로 본다. 서버가
# 돌려준 문구가 manifest.json 의 text 와 다르면 프롬프트 인젝션으로 보고
# 거부한다 — 화면에는 "content validation failed" 로만 나온다. 그래서 조건
# 분기로 문구를 바꾸지 않고, 인자만 끼워 넣는 고정 틀을 쓴다.
#
# 선택 인자를 두지 않는다. Desktop 은 **사용자가 채운 인자만** 치환하므로,
# 비워 둔 선택 인자는 ${arguments.x} 리터럴 그대로 기대값에 남는다. 서버가
# 빈 문자열로 바꾸면 그 순간 어긋나 거부당한다. 인자는 필수 하나뿐이다.
#
# manifest.json 의 text 가 이 값과 같은지는 tests/test_version.py 가 본다.
PROMPT_TEMPLATE = """\
메리츠 Open API 로 다음을 하려 합니다: ${arguments.task}

아래 순서로 진행해 주세요.
1. api_conventions() 로 인증·호출 규약과 알려진 문제를 확인합니다.
2. search_api() 로 알맞은 API 를 찾습니다. api_type 을 짐작하지 마세요.
3. get_api_detail(api_type) 로 파라미터를 확인합니다.
4. generate_code(api_type) 로 코드를 받습니다.

종목코드 표기 — 국내 시세는 6자리(005930), 국내 주문은 A+6자리(A005930),
해외는 대문자 심볼(AAPL), 해외 실시간은 <거래소>@<종목>(OQ@NVDA).

주의 — 결과가 주문·환전 API 라면 실제로 체결될 수 있습니다. 생성 코드는
confirm=True 전까지 보내지 않으니, 먼저 내용을 확인하고 사용자에게 알린 뒤
진행하세요."""


def render_prompt(task: str) -> str:
    """고정 틀에 인자만 끼워 넣는다. 분기도 덧붙임도 없다."""
    return PROMPT_TEMPLATE.replace("${arguments.task}", task)


@mcp.prompt(name="meritz_code_prompt",
            description="작업 설명만 넣으면 검색→상세→코드 생성 흐름을 안내합니다.")
def meritz_code_prompt(task: str) -> str:
    ...
    Args:
        task: 무엇을 하고 싶은지 한 줄로 적습니다. 종목이 정해져 있으면 함께
            적으십시오. 예) 삼성전자 005930 현재가를 조회하고 싶다 /
