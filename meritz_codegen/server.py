...
종목코드(비어 있으면 아직 정하지 않은 것입니다): ${arguments.iscd}
...
def render_prompt(task: str, iscd: str = "") -> str:
    """고정 틀에 인자만 끼워 넣는다. 분기도 덧붙임도 없다."""
    return (PROMPT_TEMPLATE
            .replace("${arguments.task}", task)
            .replace("${arguments.iscd}", iscd))
...
def meritz_code_prompt(task: str, iscd: str = "") -> str:
    ...
    Args:
        ...
        iscd: 종목이 정해져 있으면 넣습니다. 비워 두셔도 됩니다.
            ...
            해외는 대문자 심볼(AAPL)
    """
    return render_prompt(task, iscd)
