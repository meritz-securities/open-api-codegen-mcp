...
# 다만 **자료를 담고도 5820 이 온다**(담보 현황·실현손익). 코드만 보고
...
# **접수되지 않은 것**이다. warn_msg 를 사용자에게 보이고 warn_cnfr_yn="Y" 로
# 같은 요청을 다시 보내야 접수된다.
# 재전송(warn_cnfr_yn="Y")으로 접수되는 값. 국내와 해외의 체계가 다르다.
...
_WARN_CLASS = '''
_RESEND_OK = frozenset(%r)   # 이 값이면 warn_cnfr_yn="Y" 로 재전송하면 접수된다
...
class MeritzWarning(MeritzError):
    ...
    같은 요청을 warn_cnfr_yn="Y" 로 다시 보내야 접수된다.
    """
