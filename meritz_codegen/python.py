...
포털 예시가 실제와 다른 API 는 `corrections.json` 의 값을 써서 **되는 코드**를 만든다.
포털을 그대로 베끼면 고객이 IGW50004 를 만난다.
"""
...
def _warnings(api, catalog) -> list[str]:
    """포털 예시가 실제와 다른 지점을 코드 머리말에 적는다."""
    if catalog is None:
        return []
    out = []
    for fx in catalog.corrections_for(api.key):
        out += ["", f"주의 — 포털 등록분에 문제가 있습니다: {clean_text(fx['kind'])}"]
        if fx.get("if_you_follow_the_portal"):
            out.append(f"  포털 예시대로 하면: {clean_text(fx['if_you_follow_the_portal'])}")
...
def build_rest(api, catalog=None) -> str:
    ...
    if is_post:
        ...
        shown = ("payload" if not is_oauth else
                 '{_k: ("***" if _k in ("client_secret", "token") else _v)\n'
    ...
    tail = ["", "", 'if __name__ == "__main__":',
            f'    result = {fn}({", ".join(call)})', "    print(result)"]
    if is_post:
...
''' % (PROD, PROD_WS, tr_cd)
    ...
    tail = ["", "", 'if __name__ == "__main__":', f"    asyncio.run({fn}({key_ex}))"]
    return "\n".join(["\n".join(head), pre, "\n".join(b), "\n".join(tail)])
