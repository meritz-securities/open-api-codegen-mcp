...
ALIASES: dict[str, list[str]] = {
    ...
    "거래내역": ["transactions"], "입출금": ["transactions"], "이체": ["transactions"],
    "계좌": ["holdings", "deposit", "valuation"],
    "외화잔고": ["ovs_currencies"],
    # 시세
    "현재가": ["market_prices", "ovs_market_prices"],
    "주가": ["market_prices", "ovs_market_prices"],
    "가격": ["market_prices", "ovs_market_prices"],
    "시세": ["market_prices", "market_orderbook", "ovs_market_prices"],
    ...
    "사기": ["orders_buy"], "팔기": ["orders_sell"],
    "예약": ["reserved_orders"], "예약주문": ["reserved_orders"],
    "신용": ["credit_orders_buy", "credit_orders_sell"],
    "신용융자": ["credit_orders_buy"], "대주": ["credit_orders_sell"],
    "알림": ["ws_cntg_rslt", "ws_stck_cntg"], "통보": ["ws_cntg_rslt"],
    ...
    # 인증 — 별칭이 없으면 "토큰"·"인증" 같은 말로 이 두 건에 닿지 않는다
    "토큰": ["oauth2_token", "oauth2_revoke"],
    "토큰발급": ["oauth2_token"], "접근토큰": ["oauth2_token"],
    "인증": ["oauth2_token"], "로그인": ["oauth2_token"],
    "앱키": ["oauth2_token"], "oauth": ["oauth2_token", "oauth2_revoke"],
    "토큰폐기": ["oauth2_revoke"], "폐기": ["oauth2_revoke"],
    "revoke": ["oauth2_revoke"],
}

CATEGORY_KO = {
    "account": "계좌", "domestic_market": "국내주식 시세", "trading": "주문",
    "forex": "환전", "reference": "기준정보", "realtime": "실시간(웹소켓)",
    "overseas_market": "해외주식 시세", "oauth2": "인증(접근토큰)",
}
...
SCOPES = {
    ("해외", "미국", "미주", "나스닥", "뉴욕", "홍콩", "도쿄", "상해", "심천",
     "nasdaq", "nyse", "amex"):
        lambda a: "overseas" in a.path or a.key.startswith(("ovs_", "ws_exst_")),
    ("외화",): lambda a: a["category"] == "forex" or "currencies" in a.path,
    ("국내", "krx", "코스피", "코스닥"):
        lambda a: "overseas" not in a.path and not a.key.startswith("ovs_"),
    ("실시간", "스트리밍", "웹소켓"): lambda a: a.is_websocket,
}
...
def search(catalog, keyword: str, category: str | None = None) -> list[dict]:
    ...
    scopes = [f for words, f in SCOPES.items()
              if any(w.replace(" ", "").lower() in kw for w in words)]
    ...
    SPECIFIC = ("매수", "매도", "정정", "취소", "환전", "체결통보", "가능", "예약")
    ...
    # 범위어가 걸러낸 결과가 없을 때, **범위보다 키워드를 먼저 푼다.**
    #
    # "해외 잔고" 는 어느 API 이름에도 그 순서로 붙어 있지 않다("해외주식 잔고 조회").
    # 예전에는 곧장 범위를 풀어 국내 holdings 를 1위로 내놓았다. 범위어를 명시한
    # 질의에 범위 밖 API 를 주는 것은 틀린 답이지 넓은 답이 아니다.
    # 먼저 범위어를 뺀 나머지("잔고")로 범위 안에서 다시 찾는다.
    if not hits and scopes:
        residual = kw
        for words in SCOPES:
            for w in words:
                residual = residual.replace(w.replace(" ", "").lower(), "")
        if residual and residual != kw:
            for word, types in sorted(ALIASES.items(), key=lambda kv: rank(kv[0])):
                w = word.replace(" ", "").lower()
                if w in residual or residual in w:
                    for t in types:
                        api = catalog.get(t)
                        if api:
                            add(api, f"별칭 '{word}'")
            for api in catalog:
                if residual in text_of(api).replace(" ", "").lower():
                    add(api, "본문")

    # 아직 없으면 질의를 낱말로 쪼개 다시 본다.
    #
    # 질의를 통짜 문자열로만 비교하면 아는 말이 섞여 있어도 못 찾는다.
    # "삼성전자 주가" 는 "삼성전자주가" 가 되어 어느 별칭에도 안 걸렸다.
    # 종목명·군더더기는 버리고 아는 낱말로 찾는다.
    if not hits:
        tokens = [t for t in (keyword or "").lower().split() if len(t) >= 2]
        for token in tokens:
            for word, types in sorted(ALIASES.items(), key=lambda kv: rank(kv[0])):
                w = word.replace(" ", "").lower()
    ...
    # 그래도 없으면 그때 범위를 푼다. 빈 결과보다는 넓은 결과가 낫다.
    if not hits and scopes:
        scopes.clear()
        for word, types in sorted(ALIASES.items(), key=lambda kv: rank(kv[0])):
            w = word.replace(" ", "").lower()
