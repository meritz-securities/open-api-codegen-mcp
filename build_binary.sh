...
#
#   ./build_binary.sh            → bin/meritz-codegen-mcp       (Mac·Linux)
#   ./build_binary.sh            → bin/meritz-codegen-mcp.exe   (Windows, Git Bash)
#
...
from meritz_codegen.server import main
...
pyinstaller --onefile --name "$NAME" --distpath "$OUT" --workpath build \
  --collect-all fastmcp --collect-submodules mcp --collect-all pydantic --collect-all meritz_codegen \
...
env = {"PATH": "/usr/bin:/bin" if os.name != "nt" else os.environ.get("SystemRoot", "C:\\Windows") + "\\System32",
       "HOME": os.environ.get("HOME", ""), "SYSTEMROOT": os.environ.get("SystemRoot", ""), "MCP_TYPE": "stdio",
       }
