...
def main():
    ...
    want = expected_sha256(here, name)
    if want is None:
    ...
    if reg in cmds:
        # 셸을 거치지 않고 인자 목록으로 실행한다. 경로에 공백이나 따옴표가 있어도 그대로 전달된다.
        argv = {
            "claude-code": ["claude", "mcp", "add", SERVER_NAME, "--", dst],
            "codex":       ["codex", "mcp", "add", SERVER_NAME, "--", dst],
            "gemini":      ["gemini", "mcp", "add", SERVER_NAME, dst],
        }[reg]
        print("등록 실행: %s" % " ".join(argv))
    ...
    print("  Cursor·VS Code : docs/install.md 의 JSON 설정을 각 앱의 MCP 설정 파일에 넣으십시오")
