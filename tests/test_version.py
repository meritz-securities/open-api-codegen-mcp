...
def test_rendered_prompt_matches_the_declared_template_after_substitution():
    """치환한 결과가 선언과 어긋나지 않는지. 분기를 넣으면 여기서 걸린다."""
    from meritz_codegen.server import PROMPT_TEMPLATE, render_prompt

    manifest = json.loads((ROOT / "manifest.json").read_text(encoding="utf-8"))
    declared = next(p for p in manifest["prompts"] if p["name"] == "meritz_code_prompt")
    for task, iscd in (("현재가 조회", "005930"), ("주문", ""), ("", "")):
        expected = (declared["text"]
                    .replace("${arguments.task}", task)
                    .replace("${arguments.iscd}", iscd))
        assert render_prompt(task, iscd) == expected, f"({task!r}, {iscd!r}) 에서 어긋납니다"
    # 선언한 인자 말고 다른 치환 자리가 남아 있으면 안 된다
    leftover = set(re.findall(r"\$\{arguments\.(\w+)\}", PROMPT_TEMPLATE)) - set(declared["arguments"])
    assert not leftover, f"선언되지 않은 치환 자리: {sorted(leftover)}"
