...
class SearchTest(unittest.TestCase):
    ...
    def test_response_handling_note_is_surfaced_in_search(self):
        """응답 처리 참고사항이 있는 API는 검색 결과에 표시되어야 한다.
        ...
        self.assertIn("response_handling_note", hits[key])
...
class OrderWarningTest(unittest.TestCase):
    ...
    def test_resend_instruction_is_present(self):
        """경고 시 warn_cnfr_yn="Y" 로 재전송해야 한다는 안내가 있어야 한다."""
        code = gen(CAT.get("orders_buy"))
        self.assertIn('warn_cnfr_yn="Y"', code)
